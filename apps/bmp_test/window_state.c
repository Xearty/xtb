...
create_window_state(Vector2 screen_size, Vector2 virtual_size)
{
    ...
    state.scale = 1.0f / Min(screen_size.x / virtual_size.x,
...
update_mouse_position(WindowState *state)
{
    ...
    state->virtual_mouse_position.x = Clamp(state->virtual_mouse_position.x,
                                            0,
                                            state->screen_size.x);
    state->virtual_mouse_position.y = Clamp(state->virtual_mouse_position.y,
                                            0,
                                            state->screen_size.y);
}
