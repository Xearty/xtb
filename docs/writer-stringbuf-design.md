...
`xtb.fmt.writer` must not import `xtb.string`. This makes it possible for
`StringBuf` to depend on the generic writer without a module cycle.

`xtb.fmt.print` publicly re-exports `xtb.fmt.writer`, preserving the natural
...
`xtb.format` is a thin value-composition layer over `Writer`. Its
...
higher-level decorators such as `xtb.ansi.styled(value, style)` compose
...
`xtb.fmt.print.fileWriter(FILE*)` adapts libc output to `Writer`. libc retains
...
- StringBuf sink adapters from `xtb.fmt.print`;
