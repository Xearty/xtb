...
The repository now includes `just benchmark pools`. Its current benchmark set covers dense/sparse Pool iteration, direct index scanning, dense/sparse GenerationalPool state scanning, recycled allocation/deallocation, generational handle lookup, and a 10-million-slot reservation with only 16 live values. Scan cases are rendered as aligned tables and report occupancy plus three complementary metrics: ns/live item, ns/provisioned slot, and us/full scan. `ns/live item` measures cost per useful value yielded to the caller; `ns/provisioned slot` measures the underlying traversal work independently of occupancy; and `us/full scan` makes total traversal latency explicit. ANSI-capable terminals receive structural section/header color only; `NO_COLOR`, `TERM=dumb`, and redirected output stay plain. Dense and sparse results should be interpreted together through all three metrics rather than through live-item normalization alone.
 Scan timings use a warm-up and then report the median of five longer samples; the header prints scans per sample and warm-up count so timing duration is explicit rather than hidden.

Plain Pool also reports nonzero and zero occupancy-word counts and has two equal-occupancy sparse fixtures. The fixture geometry uses Pool's actual indexing rule rather than a zero-based approximation: because index 0 is reserved, capacity 4096 covers bitmap indices 0 through 4096 and therefore provisions 65 64-bit occupancy words; slot `index` maps to word `index / 64`. The interleaved fixture keeps every eighth slot live, leaving all 65/65 occupancy words nonzero and giving `items()` no whole word to skip. The clustered fixture keeps actual words 0, 8, ..., 64 live and clears the seven words between them, yielding the same 512 live slots with 9/65 nonzero words and 56/65 whole zero words. Benchmark setup derives its displayed bitmap statistics from the live indices and validates the equal-live-count/nonzero-word invariants before timing, preventing labels from drifting away from the actual Pool layout.

The benchmark intentionally remains opt-in and reports workload-specific timings rather than defining ABI/API guarantees. An optimized LDC code-generation probe also confirms that `GenerationalPool.items()` is fully inlined: the active high bit lowers directly to a signed-state test and no range-method call remains in the loop.
