...
import xtb.math.vector : Vector2, Vector3, Vector4, cross, dot, normalized,
    withW, xyz;
...
{
    return Matrix3(Vector3(m.c0.x,m.c1.x,m.c2.x),
        Vector3(m.c0.y,m.c1.y,m.c2.y), Vector3(m.c0.z,m.c1.z,m.c2.z));
}

pure Matrix4 transposed(Matrix4 m)
{
    return Matrix4(Vector4(m.c0.x,m.c1.x,m.c2.x,m.c3.x),
        Vector4(m.c0.y,m.c1.y,m.c2.y,m.c3.y),
        Vector4(m.c0.z,m.c1.z,m.c2.z,m.c3.z),
...
pure @system
bool tryInverse(Matrix2 m, Matrix2* output)
{
...
pure @system
bool tryInverse(Matrix3 m, Matrix3* output)
{
    assert(output !is null);
    const d=m.determinant;
    if (d == 0) return false;
...
pure @system
bool tryInverse(Matrix4 m, Matrix4* output)
{
    assert(output !is null);
    float[8][4] rows;
    ...
    foreach (column; 0 .. 4)
    {
        ...
        if (pivotMagnitude == 0) return false;
        if (pivot != column)
        {
        ...
        foreach (entry; 0 .. 8) rows[column][entry] /= divisor;
        foreach (row; 0 .. 4)
        {
...
pure @system
bool tryAffineInverse(Matrix4 m, Matrix4* output)
{
    assert(output !is null);
    if (!m.isAffine) return false;
    Matrix3 linearInverse;
    if (!Matrix3(m.c0.xyz,m.c1.xyz,m.c2.xyz).tryInverse(&linearInverse)) return false;
...
pure Matrix4 translated(Matrix4 base, Vector3 offset) { return translation(offset)*base; }
pure Matrix4 scaled(Matrix4 base, Vector3 factors) { return scaling(factors)*base; }
pure Matrix4 scaled(Matrix4 base, float factor) { return scaling(factor)*base; }
Matrix4 rotated(Matrix4 base, Vector3 axis, float angle) { return rotation(axis,angle)*base; }
...
Matrix4 orthographic(float left, float right, float bottom, float top, float near, float far)
{
    ...
    return Matrix4(Vector4(2/(right-left),0,0,0),Vector4(0,2/(top-bottom),0,0),
        Vector4(0,0,-2/(far-near),0),Vector4(-(right+left)/(right-left),
...
Matrix4 lookAt(Vector3 eye,Vector3 target,Vector3 up)
{
    ...
    return Matrix4(Vector4(side.x,correctedUp.x,-forward.x,0),
        Vector4(side.y,correctedUp.y,-forward.y,0),
        Vector4(side.z,correctedUp.z,-forward.z,0),
        Vector4(-dot(side,eye),-dot(correctedUp,eye),dot(forward,eye),1));
}

private pure bool close(float a,float b,float epsilon=0.0001f)
{
    const d=a-b; return d < epsilon && d > -epsilon;
}
