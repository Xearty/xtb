module xtb.core.containers.hash_map;
...
version (unittest) import xtb.core.containers.hash_set;
...
package(xtb.core.containers) struct OwnedHashMapElementOps(T)
{
nothrow @nogc:
...
package(xtb.core.containers) template isSimpleHashValue(T)
{
    enum isSimpleHashValue = __traits(isCopyable, T) &&
        !needsDeinit!T && !hasDDestructor!T;
}

package(xtb.core.containers) template IsDefaultHashPolicy(Hasher, K)
{
...
package(xtb.core.containers) template IsDefaultEqualPolicy(Equal, K)
{
...
package(xtb.core.containers) void requireValidHashAllocator(Allocator* allocator) @trusted
{
    version (XTB_Checked)
