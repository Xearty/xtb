module xtb.sync.internal.generation_wait;
...
nothrow @nogc:
    ...
    package(xtb.sync) void waitForChange(uint expected) @safe
    {
        version (XTB_Checked)
            registerWaiter();
    ...
    package(xtb.sync) void waitForChangeRegistered(uint expected) @safe
    {
    ...
    version (XTB_Checked)
    {
        ...
        package(xtb.sync) bool hasWaiters() const @safe
        {
            return waiters_.load(MemoryOrder.acquire) != 0;
        }

        package(xtb.sync) void registerWaiter() @safe
        {
        ...
        package(xtb.sync) void unregisterWaiter() @safe
        {
            size_t observed = waiters_.load(MemoryOrder.relaxed);
            while (true)
            {
