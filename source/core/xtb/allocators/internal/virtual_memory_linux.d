...
package(xtb.allocators.internal) enum bool virtualMemorySupported = true;

package(xtb.allocators.internal) size_t virtualMemoryPageSizeBackend() @system
{
...
package(xtb.allocators.internal) void* tryReserveVirtualMemoryBackend(size_t bytes) @system
{
    void* result = mmap(
        null,
        bytes,
...
package(xtb.allocators.internal) bool tryCommitVirtualMemoryBackend(void* address, size_t bytes) @system
{
    return mprotect(address, bytes, PROT_READ | PROT_WRITE) == 0;
}

package(xtb.allocators.internal) bool tryDecommitVirtualMemoryBackend(
    void* address,
    size_t bytes,
) @system
{
...
package(xtb.allocators.internal) bool releaseVirtualMemoryBackend(void* address, size_t bytes) @system
{
