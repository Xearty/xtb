...
import xtb.memory : Allocator, allocate_array, allocate_zeroed_array;
...
) @system
{
    ...
    *output = NativeProcessId.init;
    ...
    const(char)** argv = scratch.allocator.allocate_array!(const(char)*)(argvCount).ptr;
    argv[0] = argumentZero;
...
) @system
{
    ...
    const(char)** result = allocator.allocate_array!(const(char)*)(capacity).ptr;
    bool* emitted = allocator
        .allocate_zeroed_array!bool(environment.entries.length).ptr;
...
) @system
{
    ...
    char* result = allocator.allocate_array!char(length + 1).ptr;
...
{
    char* result = allocator.allocate_array!char(source.length + 1).ptr;
    if (source.length != 0)
